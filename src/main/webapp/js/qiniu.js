...
// use cookie to make a polyfill
    ...
    function QiniuJsSDK() {
        ...
        if (window.location.protocol === 'https:') {
            qiniuUploadUrl = 'https://up-z1.qiniu.com';
        } else {
...
function formetFileSize(fileSize) {// 转换文件大小
    var fileSizeString = "";
    if (fileSize < 1024) {
        fileSizeString = fileSize + "B";
    } else if (fileSize < 1048576) {
        fileSizeString = (fileSize / 1024).toFixed(2) + "K";
    } else if (fileSize < 1073741824) {
        fileSizeString = (fileSize / 1048576).toFixed(2) + "M";
    } else {
        fileSizeString = (fileSize/ 1073741824).toFixed(2) + "G";
    }
