...
public class TokenManager {
	//用户登录管理
	@Autowired
	public static SampleRealm realm;
	/**
	 * 获取当前登录的用户User对象
	 * @return
	 */
	 ...
	 * @return
	 */
	public static SystemUser login(SystemUser systemUser, Boolean rememberMe){
		ShiroToken token = new ShiroToken(systemUser.getUserName(), systemUser.getPassword());
	 ...
	 * @return
	 */
	public static SystemUser loginByWx(SystemUser user, Boolean rememberMe){
		ShiroToken token = new ShiroToken(user.getUserName(), user.getPassword(), "1", user.getUserBianhao());
		token.setRememberMe(rememberMe);
		SecurityUtils.getSubject().login(token);
		return getToken();
	}
	public static SystemUser loginByBianhao(SystemUser user, Boolean rememberMe){
		ShiroToken token = new ShiroToken(user.getUserName(), user.getPassword(), "2", user.getUserBianhao());
		token.setRememberMe(rememberMe);
		SecurityUtils.getSubject().login(token);
		return getToken();
	}
